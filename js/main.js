...
function getBooks(year) {
  var query = {
    ...
    body: {
      ...
      "query": {
        "bool": {
          "filter": {
            "or": [{ "range": { "end_date": { "from": year + "-01-01", "to": year + "-12-31" } } }, { "range": { "start_date": { "from": year + "-01-01", "to": year + "-12-31" } } }, { "range": { "buy_date": { "from": year + "-01-01", "to": year + "-12-31" } } }]
          }
        }
      }
    }
  ...
  var transformFunc = function transformFunc(res) {
    return {
      books: res.hits.hits.map(function (h) {
        return h._source;
      })
    };
  };
